...
//不推荐 func Base64_encode_str(str string) *string {
// 	str_ret := base64.StdEncoding.EncodeToString([]byte(str))
// 	return &str_ret
// }
// func Base64_decode_str(str string) *string {
...
func Base64_encode_byte(str []byte) []byte {
	buf := make([]byte, base64.StdEncoding.EncodedLen(len(str)))
	base64.StdEncoding.Encode(buf, str)
	return buf
}
func Base64_decode_byte(bytes []byte) []byte {
	dbuf := make([]byte, base64.StdEncoding.DecodedLen(len(bytes)))
	n, err := base64.StdEncoding.Decode(dbuf, bytes)
	if err != nil {
		panic(err)
	}
	return dbuf[:n]
